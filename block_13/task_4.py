...
# программа выдаёт ошибку. Если всё нормально, то в каждом числе первая и последняя цифры меняются местами, а затем выводится их сумма.
...
# main — функция ничего не получает на вход, внутри она запрашивает нужные данные от пользователя, выполняет дополнительные
# проверки и вызывает функции 1 и 2 для выполнения задачи (проверки и изменения двух чисел).
...
def change_number(num):
    last_d = str(num % 10)
    first_d = str(num // 10 ** (count_numbers(num) - 1))
    between = str(num // 10 % 10 ** (count_numbers(num) - 2))
    return int(last_d + between + first_d)

def main():
    first_n = int(input('Введите первое число: '))
    second_n = int(input('Введите второе число: '))
    if count_numbers(first_n) >= 3 and count_numbers(second_n) >= 4:
        return f'Сумма измененных чисел {change_number(first_n)} + {change_number(second_n)} = {change_number(first_n) + change_number(second_n)}'
    return 'Ошибка ввода, первое число минимум 3 цифры, второе минимум 4'
